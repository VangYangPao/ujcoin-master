...
// ***TODO*** ScanHash is not yet used in Zula
//
...
{
    LogPrintf("ZulaMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("zula-miner");
    ...
    try {
        ...
        while (true) {
            ...
            */
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("ZulaMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("ZulaMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("ZulaMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("ZulaMiner -- runtime error: %s\n", e.what());
        return;
    }
}
