...
const char * const BITCOIN_CONF_FILENAME = "zula.conf";
const char * const BITCOIN_PID_FILENAME = "zulad.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "zula" is a composite category enabling all Zula-related debug output
            if(ptrCategory->count(string("zula"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ZulaCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ZulaCore
    // Mac: ~/Library/Application Support/ZulaCore
    // Unix: ~/.zulacore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ZulaCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ZulaCore";
#else
    // Unix
    return pathRet / ".zulacore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty zula.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override zula.conf
